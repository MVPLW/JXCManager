...
public class export {
	...
	public void export1(OutputStream out) throws Exception {
		try {
			 ...
			 * (rowName.length-1)));
			 */// 合并单元格
			sheet.addMergedRegion(new CellRangeAddress(0, 1, 0, dataList.get(0).length - 1));// 列行
...
//		output.flush();
	...
	// registrationInformationDao.findRegistrationInformation1(map);// 获取数据
	...
	// + new String((findByName.getParament_value() + /* 下载后的文件名
	...
	// /* 表的标题 */ new String[] { findByName.getParament_value()/* 内容标题 */ },
