						...
						<div class="control-group">
							<div data-condition="search">
								报损单号:<input type="text" name="damagedRequestId"
									class="input-medium" placeholder="请输入报损单号" value="${damagedRequestId }"/> 制单人: 
									<input type="text" name="requestEmp" class="input-medium" placeholder="请输入制单人" value="${requestEmp }"/>								
							...
							<table
								...
								<tbody id="productBody">
									<c:forEach var="d" items="${damged.list}">
										<tr>
											<th><input type="checkbox" name="productCheck" value="${d.damagedRequestId}" /></th>											
											<td>${d.damagedRequestId}</td>
											<td class="center">${d.requestEmp.empLoginName}</td>											
											<td class="center"><fmt:formatDate value="${d.requestTime}" pattern="yyyy-MM-dd" /></td>
											...
											<c:if test="${d.orderStatus.no !=3 && d.orderStatus.no !=4 && d.orderStatus.no !=6 && d.orderStatus.no!=7 && d.orderStatus.no!=5}">
												<a href="javascript:;" onclick="javascript:location.href='gorequisitionupdate?requisitionId=${d.damagedRequestId}';">编辑</a>
											</c:if>
											<c:if test="${ d.orderStatus.no !=3 && d.orderStatus.no !=4 && d.orderStatus.no !=5 && d.orderStatus.no !=6 && d.orderStatus.no !=7}">
												<a href="#" id="deptreview" onclick="deptreview('${d.damagedRequestId}')">审核</a>
											</c:if>
											</td>
										</tr>
									</c:forEach>
